...
async function listUsers({ fromDate, toDate }) {
  ...
  while (nextUrl) {
    if (++pages > 20) {
      throw new Error('Eber member listing exceeded the 20-page safety limit');
    }
    nextUrl.searchParams.set('from_date', fromDate);
    nextUrl.searchParams.set('to_date', toDate);
    nextUrl.searchParams.set('limit', '100');
    if (visitedUrls.has(nextUrl.toString())) {
      throw new Error('Eber member listing repeated a pagination URL');
    }
    ...
    if (Number.isInteger(payload.total) && users.length >= payload.total) {
      break;
    }
